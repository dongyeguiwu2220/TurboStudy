...
public class EnrollService extends BaseMyCustService {
    ...
    public String getEnrollStatusDesc(int status, long start, long end, long joinCnt, long maxCnt) {
        ...
        if (status == EnrollModel.STATUS.STOP) return "选课停止";
        else if (NumberUtil.compare(start, time) > 0) return "选课未开始";
        else if (NumberUtil.compare(end, time) < 0) return "选课结束";
        else if (joinCnt >= maxCnt) return "课程已满";
        else return "选课中";
    }
    ...
    public Map<String, Object> detailForEnrollJoin(long userId, long enrollId) {
        ...
        if (ObjectUtil.isEmpty(ret)) throw new AppException("该课程不存在");
    ...
    public void cancelMyEnrollJoin(long userId, long enrollJoinId) {
        ...
        if (ObjectUtil.isEmpty(enrollJoin)) throw new AppException("未找到可取消的选课记录");
        ...
        if (NumberUtil.compare(enroll.getEnrollEnd(), TimeHelper.timestamp()) < 0)
            throw new AppException("该课程选课已经结束，无法取消");
